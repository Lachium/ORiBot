...
public:
	A_Star(/*shared_ptr<Point> pGlobalPos*/)
	{
	...
	void doPathdindingB(int xA, int yA, int xB, int yB, const deque<deque<MapTile>>& gridMap)
	{
		...
		// get the route
		clock_t start = clock();
		string route = pathFind(xA, yA, xB, yB, mapArr);
		if (route == "") cout << "An empty route generated!" << endl;
		clock_t end = clock();
		double time_elapsed = double(end - start);
		cout << "Time to calculate the route (ms): " << time_elapsed << endl;
		...
		if (route.length() > 0)
		{
			...
			{
				for (int x = 0; x < mapArr.front().size(); x++)
					if (mapArr[y][x] == 0)
						cout << ".";
